...
#[derive(Debug)]
pub struct HyperChainProcessor {
    verbose: bool,
    processed_count: usize,
}

impl HyperChainProcessor {
...
pub fn run(verbose: bool, input: Option<String>, output: Option<String>) -> Result<()> {
    ...
    info!("Starting HyperChain processing");
    
    let mut processor = HyperChainProcessor::new(verbose);
...
mod tests {
    ...
    fn test_processor_creation() {
        let processor = HyperChainProcessor::new(true);
    ...
    fn test_data_processing() {
        let mut processor = HyperChainProcessor::new(false);
